...
export default function App() {
  ...
  // --- FITUR 2: SUTRADARA AI (Multimodal + Playbook) ---
  ...
  // --- FITUR 3: GENERATOR PROMPT ASET (Youri's standard) ---
            ...
            <div className="grid grid-cols-2 gap-4">
              {tags.map((tag, i) => (
                <div key={i} className="bg-gray-900 p-4 rounded-xl border border-gray-800 flex justify-between items-center">
                  <div>
                    <h3 className="font-bold text-blue-400">{tag.tag}</h3>
                    <p className="text-xs text-gray-500 line-clamp-2">{tag.desc}</p>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => handleGenerateAssetPrompt(tag.type, tag.tag)} className="text-yellow-500 hover:text-yellow-400" title="Prompt 3-Panel"><ImageIcon size={18}/></button>
                    <button onClick={() => setTags(tags.filter((_, idx) => idx !== i))} className="text-red-500 hover:text-red-400"><Trash2 size={18}/></button>
                  </div>
                </div>
              ))}
            </div>
